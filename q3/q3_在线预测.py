...
# 1. 读取数据
data = pd.read_csv(r"C:\Users\Lenovo\WPSDrive\1628475303\WPS企业云盘\同济大学\我的企业文档\我的企业文档\大二下\数学建模\作业3\q3_所有用户在线特征_天.csv")
# 转换时间戳为datetime类型
...
# 3. 构建训练数据：使用过去7天的数据作为特征，2024/7/20的在线情况作为标签
...
# 8. 使用训练好的模型来预测目标用户在2024/7/21的在线情况
...
for i, user_id in enumerate(exclude_user_ids):
    print(f"Predicted online status for {user_id} on 2024/7/21: {predictions_label[i].item()}")
